...
系统通知：
- `notifications.enabled=true` 时，会在支持的终端中发送本地通知
- `notifications.onTurnEnd` 控制普通回合结束通知
- `notifications.onLoopResult` 控制显式或自动 loop 的终态通知
- 环境变量也可覆盖：`GLM_NOTIFY_ENABLED`、`GLM_NOTIFY_ON_TURN_END`、`GLM_NOTIFY_ON_LOOP_RESULT`
- 可选声音 hook：`GLM_NOTIFY_SOUND_CMD='...'`
...
- `proxy`：只注册一个 `mcp__<server>__proxy` 代理工具，首次真正调用时再懒连接 server。
...
`hybrid` 模式的首次冷启动如果没有有效缓存，会先以 `proxy` 方式提供服务；等首次成功连上并刷新缓存后，执行 `/mcp reload` 或重开会话即可切到缓存直出工具。
